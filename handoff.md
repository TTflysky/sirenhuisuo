...
## v2.2.0 专家团编排升级（2026-07-30）

- `v2.2.1` 补充：全部 268 位内置专家在首次启动时自动迁移为真实办公室员工，稳定分配职责名称、部门、头像框和工位；旧的 999 个固定空工位已改为按实际员工数量生成。

- 本地版本已升级至 `2.2.0`，Windows 安装包为 `release/taiji-office-setup-2.2.0.exe`。
- 已内置 `jnMetaCode/agency-agents-zh` 的 268 位 MIT 许可中文专家，包含来源、许可证、职责摘要和完整规则；目录专家不会默认挤入办公室，只有被选择加入项目或团队时才物化为可执行员工。
...
- 完整设计、已完成/未完成边界、后续顺序和验证记录见 `docs/V2.2.0_AGENCY_EXPERT_ORCHESTRATION.md`。下一次继续前必须先读该文档，避免把“目录和项目简报已完成”误判为“多专家方案与 ProjectBrief DAG 已全部实现”。
