# v5.10.4 真实陪跑故障修复交接（2026-08-08）

## 本轮目标

本轮针对真实陪跑中项目没有交付、重复暂停无效、任务账本暴涨、状态与项目清单不一致、明确产物被模型压缩等通用内核问题修复。没有添加单项目关键词特例，目标仍是自主智能体团队。

## 已完成

- 任务账本超过 64MB 时先写快照再归档；启动按快照水位流式验证尾部事件并校验哈希链，支持分段账本继续恢复。
- 账本不可读时暂停、停止、关闭仍向 Worker 生效，并返回降级状态与警告。
- 事实账本只处理水位之后的新证据，历史版本达到上限后重启不再重复生成冲突。
- 通用识别明确文件名与测试报告；空壳项目对象不会清掉磁盘成员，当前明确非空成员和产物会覆盖旧快照。

## 验证

- `verify:task-runtime-store`、`verify:task-worker`、全量单测（59 文件、197 项）和 `verify:v510` 全部通过。
- 真实 AppData 已无损迁移：约 1GB `task-events.jsonl` 归档到 `task-event-archives`；当前快照序列 77519，`snapshotValid/indexValid=true`，历史未删除。

## 下一步

使用相同需求重新创建独立对话、项目和工作区，验证五项文件实际写入、运行、验收并导出回放。出现任务继续增长但无新证据、成员显示工作中却无动作、产物未归属项目或完成门无证据时，立即暂停并记录事件序列。
...
- 累计 Token 达到任务预算时，执行控制器进入 `checkpointed`，不再直接写入业务失败 `blocked`。
- 已有真实证据可以继续进行最终验收；验收通过才进入 `completed`。
- 没有足够证据时保留产物、证据、未决问题和恢复上下文，恢复时重置当前预算周期并保留 `lifetimeUsage`。
- 团队运行投影把容量检查点显示为暂停，不把成员标记成失败或继续显示工作中。
- 新增 `test/unit/executionCapacityCheckpoint.test.mjs`，完整回归为 58 个测试文件、195 项测试通过。
- V5 自评见 `docs/TAIJI_V5_SELF_ASSESSMENT.md`；当前结论是工程 Beta，尚未达到任意复杂真实项目稳定交付标准。

## 发布前状态

- 当前版本：`5.10.3`。
- `npm.cmd run verify:v510`、`npm.cmd run test:run`、`npm.cmd run verify:execution-controller`、`npm.cmd run verify:v5` 已通过。
- 尚未提交、打包或发布本轮改动；用户要求发布后才能制作办公室更新包。
...
- 已有真实证据仍可完成最终验收；没有足够证据时保留产物、未决问题和恢复上下文。
- 当前版本已通过 V5.10 内核门禁、58 个测试文件和 195 项测试；完整自评见 `docs/TAIJI_V5_SELF_ASSESSMENT.md`。
...
# v5.10.3 容量告警与最终验收分离修复（2026-08-08）

## 本轮补记

- 近期多次太极任务在收口阶段把“上下文已接近上限 / 约 48 万 Token”直接表现成“任务未完成”或“已阻塞”，容易让用户误以为是逻辑失败。
- 这个提示不该继续作为最终失败原因；更合适的做法是把它降级为“上下文容量告警”，并引导新开对话、压缩历史或切换会话继续，而不是把容量问题混同为任务结果。
- 真正的任务收口仍应围绕可验证证据：例如是否实际回读 `handoff.md` 顶部、是否确认 4 栏内容齐全、是否完成最终验收，而不是只看 token 数字。
- 本轮已把“容量告警 / 收口中断 / 最终验收失败”拆成独立状态：累计 Token 达到上限进入 `checkpointed`，已有证据仍可完成最终验收。
- 恢复时重置当前预算周期并保留 `lifetimeUsage`，避免恢复任务一启动就再次被旧 Token 数字拦截。

## 下一步

1. 继续把能力缺口接入现有团队提案 revision，自动补员后只恢复受影响节点。
2. 把计划层的 ready 并行组接入有上限的原生执行并发。
3. 用真实项目验证成员规划、返工、交付和客户端重启恢复；自评报告见 `docs/TAIJI_V5_SELF_ASSESSMENT.md`。
