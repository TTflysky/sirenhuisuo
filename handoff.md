...
- 窗口控件：助手、员工私聊和团队聊天的标题栏按钮已与主界面统一为 `32 x 32px` 正方形，锁定/最小化/最大化/关闭不会再被标题栏拉成长方形。
