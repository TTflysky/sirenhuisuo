...
`v0.24.0` 已将任务存储升级为 Schema v3：事件账本仍是唯一事实源，查询索引、校验快照、审计游标、恢复点和按事件序号确定性重建已贯通 Electron IPC。恢复只追加审计事件，不会覆盖历史；API Key、连接器凭据和用户业务数据仍不进入仓库。
...
1. `v0.25.0` 完成上下文路由、预算和长期任务恢复。
2. 后续严格按既定 `v0.26.0` 至 `v0.28.0` 顺序完成动态委派、Git Worktree 和生态闭环。
