...
`v0.26.0` 已加入动态委派协议 v1。模型工具和手动 IPC 都能向运行中或暂停任务追加子任务；子任务进入正式 Plan/Runner、任务账本和恢复链，并在团队详情展示。审查退回仅追加责任子任务修订与复审，无关已完成步骤不会回退。
...
1. `v0.27.0` 完成 Git Worktree 隔离与恢复。
2. `v0.28.0` 完成生态闭环、升级诊断、全量回归和最终统一发布。
