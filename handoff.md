...
# v5.10.2 容量告警与最终验收分离补记（2026-08-08）
...
- 后续内核改造应把“容量告警 / 收口中断 / 最终验收失败”拆成独立状态，避免同一个提示同时承担资源告警和业务判定。

## 下一步

1. 调整太极任务结束文案与阻塞判定，避免再用“48 万 Token”充当最终失败原因。
2. 保留容量上限在内部日志里的可观测性，但对外只输出“上下文过长，建议新开对话继续”。
3. 继续用真实验收证据判断任务是否完成，优先回读 `handoff.md`、产物和最终结果。
