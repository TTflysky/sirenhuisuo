...
`v0.25.0` 已加入 Context Router v1 和长期恢复胶囊。插话先分类并写入结构化上下文，再决定抢占、合并或排队；长任务接近上下文/轮次预算时先压缩与创建恢复点，最终安全暂停而不是伪完成或伪失败。交付与审查步骤没有真实证据时不能被文本回复放行。
...
1. `v0.26.0` 完成动态委派、子任务和验收回退。
2. 后续严格按既定 `v0.27.0` 至 `v0.28.0` 顺序完成 Git Worktree 和生态闭环。
