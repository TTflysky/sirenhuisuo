...
`v0.23.0` 已建立工具注册协议 v1。助手、员工私聊、团队与主进程原生任务统一登记工具来源、能力、风险、运行时、健康状态和 Schema 指纹；重名、损坏、缺参数或不可用工具在执行前被拦截。诊断中心新增工具注册健康项。API Key 与连接器凭据仍不进入注册快照、账本或 GitHub。
...
1. `v0.24.0` 建立任务状态数据库、审计查询、快照和确定性恢复。
2. 后续严格按既定 `v0.25.0` 至 `v0.28.0` 顺序完成上下文路由、动态委派、Git Worktree 和生态闭环。
