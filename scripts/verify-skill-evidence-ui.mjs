...
const checks = [
  ...
  ['explicit skill read is observable', read('src/components/skills/SkillMentionInput.tsx').includes('resolveSkillContextWithEvidence') && read('src/components/skills/SkillMentionInput.tsx').includes("action: 'read-failed'")],
...
checks.push(
  ...
  ['persona matches runtime protocol', read('src/components/settings/AssistantSettingsModal.tsx').includes('## 运行时执行协议') && read('src/components/settings/AssistantSettingsModal.tsx').includes("DEFAULT_PROMPT_VERSION = '12'")],
);
