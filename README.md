...
- 团队执行器通过版本化 Adapter 协议向主进程 Worker 上报步骤开始、完成、失败、审查结论和任务收尾；检查点严格递增并写入权威任务投影，旧渲染快照不能覆盖新进度。
- 任务详情显示最新 Adapter 检查点编号和摘要。模型与工具调用当前仍在渲染进程 Adapter v1 内执行，后续可在不改变控制协议的前提下迁入主进程。
- 聊天快速跳转轨道支持悬停预览。
...
`v0.20.0` 已完成构建与发布验收。源码和交接资料以 `main` 为准。

- [直接下载 v0.20.0 安装包](https://github.com/TTflysky/sirenhuisuo/releases/download/v0.20.0/taiji-office-setup-0.20.0.exe)
- [查看 v0.20.0 发布说明](https://github.com/TTflysky/sirenhuisuo/releases/tag/v0.20.0)

本地构建的安装程序生成在：

```text
release/taiji-office-setup-0.20.0.exe
```
