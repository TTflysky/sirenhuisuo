...
export function projectGraphToTaskSteps(graph, existingSteps = []) {
  ...
  return (graph?.nodes || []).filter((node) => node.status !== 'superseded').map((node, index) => {
    ...
    return {
      ...
      acceptanceCriteria: node.acceptanceCriteria.length ? [...node.acceptanceCriteria] : [...(prior.acceptanceCriteria || [])],
      requiredCapabilities: node.requiredCapabilities.length ? [...node.requiredCapabilities] : [...(prior.requiredCapabilities || [])],
      expectedEvidence: node.expectedEvidence.length ? [...node.expectedEvidence] : [...(prior.expectedEvidence || [])],
      outputPath: node.outputPath || prior.outputPath,
      taskContract: clone(node.taskContract || prior.taskContract),
      evidenceIds: [...(node.evidenceIds || prior.evidenceIds || [])],
