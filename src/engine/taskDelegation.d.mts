...
export interface TaskDelegation { delegationVersion: number; id: string; parentTaskId: string; parentStepId?: string; delegatedStepId: string; employeeId: string; employeeName: string; title: string; assignment: string; acceptanceCriteria: string[]; dependsOnStepIds: string[]; status: TaskDelegationStatus; selectionReason?: string; availability?: 'offline' | 'busy' | 'available'; createdAt: number; updatedAt: number; completedAt?: number; output?: unknown; error?: string; evidence?: unknown[]; revisionOfDelegationId?: string }
export function selectDelegate(members: TaskRunMemberSnapshot[], assignment: string, options?: { employeeId?: string; title?: string; requiredCapabilities?: string[] }): TaskRunMemberSnapshot | null;
