...
export interface TaskRecoveryCapsule { recoveryVersion: number; taskId: string; teamId: string; immutableGoal: string; acceptanceCriteria: string[]; status: string; phase: string; workspaceId: string; completedSteps: unknown[]; pendingSteps: unknown[]; verifiedFacts: string[]; artifacts: string[]; unresolvedIssues: string[]; steeringMessages: string[]; budget: ContextBudgetSnapshot; lastCheckpoint?: unknown; reason: string; createdAt: number; checksum: string }
