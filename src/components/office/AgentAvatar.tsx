...
export default function AgentAvatar({ employee, size = 40 }: Props) {
  ...
  const gap = size >= 48 ? 4 : 2;
  const borderWidth = Math.max(frame.width, size >= 48 ? 4 : 3);
  const frameShadow = [`0 0 0 2px ${frame.secondary}`, `0 0 0 4px ${frame.primary}`, frame.shadow].filter(Boolean).join(', ');
  return <span className="agent-avatar-frame" title={frame.name} style={{ width: size, height: size, padding: gap, borderColor: frame.primary, borderStyle: frame.borderStyle, borderWidth, boxShadow: frameShadow, background: 'var(--surface)' }}>
    {image}
