...
export default function SkillMentionInput({ value, onChange, onChangeEvent, selected, onSelectedChange, onKeyDown, onPaste, disabled, placeholder, rows = 2, className = 'chat-input', ref }: Props) {
  const [skills, setSkills] = useState<Skill[]>([]); const [open, setOpen] = useState(false); const [query, setQuery] = useState(''); const [idx, setIdx] = useState(0); const [popupHeight, setPopupHeight] = useState(220);
  ...
  return <div style={{ position: 'relative', width: '100%' }}>
    ...
    {open && candidates.length > 0 && <div className="mention-popup" style={{ height: popupHeight }}><div className="mention-popup-resize" role="separator" aria-label="调整技能列表高度" title="拖动调整展示区域" onPointerDown={(event) => { event.preventDefault(); dragRef.current = { y: event.clientY, height: popupHeight }; document.body.classList.add('skill-picker-resizing'); }} /><div className="mention-popup-head">选择技能 <span>拖动上沿调整</span></div>{candidates.map((s, i) => <button key={s.id} className={`mention-option ${i === idx ? 'active' : ''}`} onMouseEnter={() => setIdx(i)} onClick={() => choose(s)}><span>🧩</span><div className="mention-option-info"><div className="mention-option-name">{s.name}</div><div className="mention-option-title">{s.description || s.source}</div></div></button>)}</div>}
  </div>;
}
