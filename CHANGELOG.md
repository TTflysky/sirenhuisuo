...
- 修复任务事件账本达到容量后仍整体读取的问题；启动时按快照水位流式验证尾部事件，旧账本完整归档，不删除历史。
- 账本暂时不可读时暂停、停止和关闭仍能向 Worker 生效，并返回降级状态与警告。
- 防止渲染器周期保存、启动迁移和事实账本历史裁剪反复写入相同状态，减少无意义事件和界面卡顿。
- 项目交付物解析保留明确文件名与测试报告，避免多项交付被压缩成模糊结果。

## v5.10.3 (2026-08-08)

- 将累计 Token 预算达到上限从业务失败改为可恢复的容量检查点；已有真实证据仍可进入最终验收，避免“已有产物但任务被判未完成”。
- 执行控制器区分当前预算周期与跨恢复周期的累计用量；从检查点恢复时只重置当前周期预算，保留累计遥测。
- 团队运行投影把容量检查点显示为暂停并保留现场，不再把成员标记为失败或继续伪装工作中。
- 新增容量临界、证据完成、无证据恢复和非 Token 安全预算四类回归测试。
- 增加 `docs/TAIJI_V5_SELF_ASSESSMENT.md`，如实记录 V5 的能力评分、交付成熟度和未达标项。

## v5.10.2 (2026-08-08)

- 将已验证的主题可读性修复方向正式升级为 v5.10.2，统一同步版本号、安装包命名和发布资产标识。
- 继续保持运行观察团队聊天在深色与风格化主题下的文字可读性修复结果，避免新对话窗口回退到固定浅色画布或不可读文本。

## v5.10.1 (2026-08-08)

- 修复新团队聊天/运行观察窗口在深色和风格化主题下仍使用固定浅色画布，造成成员名称、恢复提示、消息正文和阶段内容出现浅底浅字的问题；运行观察区域现在统一继承当前主题的语义背景、表面、边框和文字颜色。
- 将运行观察输入区的私有颜色变量限定在对应聊天壳内，并新增视觉主题契约回归，防止普通聊天受未定义变量影响或再次写死浅色画布。

## v5.10.0 (2026-08-08)

- 团队规划由固定“需求/架构 → 设计 → 实现 → 逐步审核”改为交付物驱动：模型声明真实交付物、验收标准、依赖、输出路径、验证方式和所需能力，无依赖交付物进入同一并行组。
- 新增交付物团队规划器，根据每项交付物的能力边界选择负责人，显式报告能力缺口，并由主代理在全部前置证据完成后回读、整合和最终验证；成员提交不再直接代表项目完成。
- 每个成员步骤新增结构化任务合同，覆盖输入引用、输出类型与路径、完成条件、验证方式、模型/工具/返工预算和升级条件；合同持久化到 TaskService、自适应计划图和成员责任子任务，重启后可恢复。
- 项目草案和团队聊天的成员选择会合并全部必需交付物的能力集合；新版项目简报使用交付物 DAG，旧项目数据仍保留兼容迁移路径。
- 新增 `verify:v510-deliverable-kernel` 与 `verify:v510`，覆盖并行 ready 节点、依赖整合、任务合同投影、责任子任务绑定和重启恢复；完整回归通过 56 个测试文件、190 项测试。
- 已知边界：本版完成动态团队内核的交付物规划与事实源闭环，不宣称连续 8 小时真实客户端驻留、第三方账户矩阵或所有失败场景的自动换人已经验收。

## v5.9.1 (2026-08-08)

- 团队聊天运行观察窗按已验收 Demo 重构，统一提供“观察 / 产物 / 技能 / 回放”四个视图，显示真实执行者、等待前置、完成步骤、验收证据与最近四行动态；观察窗支持调整宽度和折叠。
- 删除团队群中重复的“新建聊天 / 发起讨论 / 发布任务”入口，保留历史记录、复制、导出、附件、技能、观察、产物与回放。
- 暂停和停止操作先在本地即时反馈，再与 Worker 权威状态对账；排队成员不再伪装成工作中，延迟到达的旧状态不能覆盖用户控制结果。
- 修复原生执行完成门禁和终态竞争：文件、代码、连接器与决策类交付分别校验必要证据，最终验收证据先持久化，再由 `run_finished` 完成终态转换。
- 项目路由禁止把模糊新需求自动挂到多个旧项目中的“最近一个”；只有明确引用项目时才允许继承。项目与团队名称改为使用模型提炼后的真实目标，不再截取用户最后一句话。
- 新增运行仪表盘、观察窗、项目命名和项目隔离回归验证；完整构建、Lint、原生执行、任务 Worker、会话隔离和完成门禁均已通过。

## v5.9.0 (2026-08-07)

- 新增统一运行轨迹：任务账本、Worker、原生执行器和操作诊断会写入同一份主进程追加式遥测账本。任务、产物和诊断的原始事实源保持不变，监控台只读取投影，不建立第二套状态。
- 设置 -> 诊断中心新增“运行监控台”：每 2.5 秒刷新当前任务、最新真实动作、错误/提醒数量和已记录 Token；事件会显示任务、执行者、模型、耗时与状态，但不展示模型隐藏推理。
- 新增“导出问题包”：导出脱敏后的运行事件、任务快照、任务账本事件、操作诊断、版本与完整性信息，便于另一台电脑复现故障链路。
- 统一遥测会移除模型隐藏推理、提示词、附件/文件正文和密钥/令牌字段；事件账本可恢复、按任务/项目查询并轮转保留。
- 新增 `verify:v59-runtime-telemetry` 与 `verify:v59`，覆盖统一事件关联、任务/工具/诊断汇总、脱敏、Token 汇总、重启读取、IPC/界面接线，以及既有 V5.6-V5.8、性能与 Electron 驻留回归。

## v5.8.1 (2026-08-07)

- 修复真实陪跑会话启动时错误采纳历史任务、历史记忆和历史 Skill 调用的问题：本轮只保留会话开始后发生的证据，旧记录会被明确丢弃并留下审计原因。
- 主进程在真实陪跑期间每 5 秒自动采集任务、记忆和 Skill 账本；即使离开诊断中心，陪跑仍会继续，重启后会恢复仍在进行的采集。
- 陪跑界面改为秒级计时，显示最近采集时间和动态运行指示；“待观察”明确表示等待本轮真实行为，不再用旧任务或模拟结果填充。
- 新增“一键验收 24 项”：在隔离的自动基准会话中覆盖全部标准场景，并与真实陪跑成绩严格分开，避免把内置回放当成真实用户验收。

## v5.8.0 (2026-08-07)

- 新增自治陪跑评测内核：24 个稳定场景覆盖项目隔离、任务完成与恢复、记忆引用与跨项目污染、Skill 灰度/停用/回滚、无必要工具调用以及大员工/多窗口驻留；评测会持久化会话、观察、审计和导出数据。
- 设置 -> 诊断中心新增“自治陪跑评测”。可开始一轮真实陪跑、刷新进度、完成本轮并导出记录；没有真实样本时显示“样本不足”，不会用回放数据伪造用户陪跑成绩。
- 内置章北海人格升级至 v29，加入真实评测协议：区分自动回放、短驻留和用户真实长期使用证据，禁止把单次成功或模型自述写成长期自治结论。
- V5.8 发布门禁新增真实 Electron 12 窗口短驻留：任务检查点先通过真实 IPC 落盘，随后验证主界面、助理、员工私聊、团队、设置和工具窗口持续存活。长任务恢复与性能增长仍由独立门禁验证，避免测试越界作结论。
- 已知限制：本版尚未获得连续 8 小时的真实用户客户端陪跑证据；该项会由诊断中心启动后的真实会话持续采集，不能由自动回放替代。

## v5.7.0 (2026-08-07)

- 自动 Skill 改为跨任务候选生命周期：单次任务只能留下候选观察，至少两个独立任务通过真实验收、路线相近且失败率低于阈值后才能进入编译。
- 复盘模型只输出结构化候选，不再直接生成可安装 Markdown；确定性编译器生成规范 `SKILL.md`、输入输出契约和 `agents/openai.yaml`。
- 新增结构、frontmatter、敏感信息、权限、依赖、静态 Dry-run、正向样例和失败样例验证；硬检查失败不会进入人工审批。
- 技能库新增“学习与审批”视图，展示正文、Diff、来源任务与证据、工具路线、权限、风险、验证报告和候选积累进度。
- 自动 Skill 批准后先灰度。助理、员工私聊和团队调用统一记录成功率与失败类型；达到失败阈值自动停用，灰度通过后转为正式启用。
- 自动 Skill 每次批准均保留版本快照并支持回滚；学习流程不能覆盖内置或手动安装 Skill。
- 章北海内置人格升级至 v28；新增 `verify:v57-skill-lifecycle` 与 `verify:v57`。

## v5.6.0 (2026-08-07)

- 学习内核进入“统一记忆账本与项目边界”阶段：主进程分层记忆升级为 schema v4，新增项目范围、事实状态、替代链、历史恢复、证据 ID、来源任务和复核字段。
- 记忆检索会记录引用的 `memoryId`、命中分数、命中理由、项目/会话/任务关联，并将引用记录保存在可审计账本中；聊天任务同时把检索记录绑定到任务证据。
- 新项目不会读取其他项目的专属记忆；带项目标签的团队/员工经验也只在对应项目中可见。组织规则与已确认用户偏好仍可跨项目参考。
- 浏览器 `localStorage` 中的旧用户画像、旧长期记忆和旧任务经验不再直接注入模型。旧数据只做兼容迁移和只读核对，其中没有可验证项目边界的旧任务经验以 `legacy` 状态保存，不参与模型上下文。
- 对话洞察提炼改为创建待审核的用户记忆提案；组织、用户、项目和未验收记忆不能被高置信度策略自动批准。只有存在真实验收证据的团队/员工程序经验才允许受限自动沉淀。
- 设置中的记忆中心新增项目范围、历史版本恢复、证据数量和最近引用记录；旧版本地记忆与任务经验明确标为只读兼容备份。
- 内置章北海人格升级至 v27，加入项目记忆隔离、引用可追溯、当前事实优先和 Skill 候选不得越过验证/审批的协议。
- 新增 `verify:v56-memory-ledger` 与 `verify:v56`，覆盖项目隔离、引用审计、版本替代、回滚、旧经验隔离、自动批准边界、重启一致性和渲染端不再直注入旧记忆。

## v5.5.3 (2026-08-07)

- 修复明确 Skill 安装命令被中文说明黏连后解析失败的问题：`npx skills add owner/repo` 现在按严格 GitHub 仓库语法识别，仓库名后可直接继续写中文目标说明。
- 新的明确 Skill 来源始终建立独立任务合同；格式错误的当前命令会保留自己的校验错误，不会串回、复用或继续昨天的安装任务、工作区与来源。
- 统一主持层只对真实外部依赖做能力门禁。原生 `install_skill` 不再被 SkillHub 市场状态提前阻断，安装器能够自行验证 GitHub、ZIP 或 SkillHub 来源。
- 聊天失败状态按实际异常分类，技能安装门禁不会再被错误呈现为“连接 AI 模型”。
- 将图片附件写入模型消息的准备逻辑抽为独立模块并增加回归，核心 Agent Loop 函数重新满足项目函数边界门禁。
- 修复正式发布门禁的自污染：发布时仅校验已提交的 SBOM 与来源证明，不再在“工作区必须干净”的校验之后重新生成它们。
- 验证通过：54 个测试文件、184 项测试、Skill 安装 E2E、统一主持/能力矩阵/任务服务、完整 V2 核心门禁、生产构建与 Lint。

## v5.5.2 (2026-08-07)

- 为助手聊天新增默认开启的“执行思路与过程”：公开展示目标理解、执行计划、当前工具动作、真实结果、根据失败作出的调整和最终验收/阻塞状态。该记录是可审计的执行摘要，不展示或存储模型私有推理内容。
- 进行中的动作会显示“进行中”，完成后原地更新为结果；长原始结果可在独立滚动区和放大视图中阅读、复制，文本随“大字号”设置同步放大。
- 修复 GitHub 多 Skill 仓库安装后在 Windows 短路径/长路径混用时被误报为“已写入但扫描不到”的问题；安装验证现在使用规范化文件系统路径比较。
- Skill 安装的明确来源、绑定候选和“继续安装”均走一次可恢复的原生安装合同，不再重新请求规划模型或陷入工具重复。
- 验证通过：原生 Skill 安装 E2E、15 项相关单元测试、生产构建、执行思路契约和真实 Electron 界面回归。

## v5.5.1 (2026-08-06)

- 修复裸 `npx skills add owner/repo` 被当成聊天解释而非安装动作的问题；明确命令和“继续安装”会保留同一真实目标并进入原生安装器。
- 原生 Skill 安装器新增 GitHub 多 Skill 仓库支持：读取仓库树、安装全部或指定技能、复制完整目录、写入包清单并逐个回读验证。
- 已绑定 Skill 候选后的“安装它”不再进入通用模型循环；确定性安装只执行一次原生工具，避免耗尽约 48 万 token 的任务预算。
- 阻止后台执行交互式 `npx skills add`/`skillhub install`，避免停在 TTY 选择或写入 Codex/Claude/Cursor 等非太极目录。
- 新增完整安装链和 Agent Loop 回归；核心门禁通过 52 个测试文件、179 项测试与 400 条任务语义样例。

## v5.5.0 (2026-08-06)

- 将团队执行接入统一 TaskService：根任务、固定成员责任子任务、执行事件、成员状态和用户插话都进入同一份可回放账本。
- 完成 V5.4 项目/会话上下文隔离与附件回放恢复；重启后可以重建根任务、责任任务、已完成步骤、执行绑定和附件路径。
- 新增真实项目验收脚本 `verify:v55-project`，覆盖产品、架构、UI、前端和 QA 阶段依赖、真实文件写入、桌面/375px 验证、审查和项目范围查询。
- 修复 Electron E2E 启动参数顺序和本地 Chromium 沙盒问题，并让 V5.5 Mock 模型依据明确的当前步骤执行，不再从完整任务文本猜测后续阶段。
- 修复停止任务在人工批准补偿后被旧的停止收尾覆盖的竞态；批准后只恢复补偿队列，真实补偿工具执行完成后再结束任务。
- 修复原生执行器验证失败后后台 Worker/定时器继续存活导致发布门禁长时间假死的问题，失败会立即返回可定位结果。
- 当前确定性门禁、构建和单元回归用于发布；真实 Electron 项目验收在本机客户端手动启动后继续，未通过前不宣称 V5.5 完成最终自主项目验收。

## v5.0.0 (2026-08-05)

- 以自主智能体协作团队为目标完成 V5 内核收口：新聊天、项目、任务、产物和工作区按稳定 ID 隔离，长期记忆仍可共享但不会污染项目边界。
- 团队提案改为可版本化生命周期；成员纠正会使旧提案失效并生成新 revision，审批卡绑定当前版本，避免旧申请挂起或重新误匹配成员。
- 新团队建立后先发布每位成员的职责、计划、依赖和风险；真实执行会更新成员计划状态。
- 群聊显示来自真实模型请求的助理状态：排队、思考中、回答中、出错和在线，并提供动态点提醒。
- 阶段完成、审查和任务终态都会形成可追溯汇报；项目事件串行写入独立工作区 `events.jsonl`，项目文档同时保存 `project.md` 和 `project.json`。
- 新增 `verify:v5` 内核门禁；构建、Lint 和全量单测通过。
- 真实陪跑报告同时记录两个尚未通过最终验收的 P0：模型 DSML 工具调用适配，以及普通助理入口绕过项目/任务/产物登记。修复前不把静态门禁通过写成自主团队已完成。

## v4.0.0 (2026-08-05)

- 完成 v3.16 -> v3.17 -> v3.18 -> v4.0 连续升级：事实账本、统一主持、能力矩阵、升级事务和发布治理形成一条可审计链路。
- 新增八域升级迁移矩阵、安装后健康检查、回滚证据和提交阻断；升级失败不会继续显示为成功。
- 新增 v4 发布清单、SBOM 和来源证明，统一检查版本、lockfile、迁移、健康、回滚和五类执行入口。
- `package.json` 与 `package-lock.json` 已同步为 `4.0.0`。源码门禁、构建和 Lint 已通过；当前尚未创建 GitHub Release，未配置代码签名时保留 SmartScreen warning。
- 升级检查、下载和重启安装入口迁移到“设置 -> 诊断中心”；首页不再显示升级按钮。更新状态广播到所有窗口并可恢复最近一次状态。
- 新增 `AGENTS.md` 与 V5 交接记录，明确自主智能体团队目标和每轮路线回顾要求。

## v3.18.0 (2026-08-05)

- 新增员工、团队、会话、任务、记忆、模型、连接器和工作区八个迁移域的升级治理矩阵。
- 更新事务在准备、健康和提交阶段验证迁移证据；安装后验证与回滚记录接入自动更新流程。

## v3.17.0 (2026-08-05)

- 新增统一主持层，统一 assistant、employee、team、worker、background 五类执行入口和动作合同。
- TaskService 持久化能力矩阵与入口身份；真实配置缺失的第三方能力在调用前被阻断或等待处理。
- 事实冲突处理贯通主进程、IPC、preload 和类型声明，旧 Autopilot 不再拥有独立执行权。

## v3.16.0 (2026-08-05)

- 新增事实版本链、冲突证据和四种冲突处理结果，避免新陈述静默覆盖旧事实。
- 工具路线记录成功率、失败率和最近结果时间；分层记忆升级到 schema v3 并加入时间衰减与迁移审计。

## v3.15.4 (2026-08-05)

- 团队大厅纳入已验收 Demo 的视觉契约：Pop 风格使用网点纸张、4px 主边框、3px 次级边框、16px 圆角和硬阴影，卡片密度与空状态布局同步。
- Acid 风格同步团队大厅的深色网格、可读对比度和绿色信号色；原版商务保持原有兼容样式。
- 视觉回归门禁新增团队大厅的 Pop/Acid 材质断言，避免新增板块再次出现功能完成但界面风格脱节。

## v3.15.3 (2026-08-05)

- 交互网页任务增加可见性、数量与 Canvas 非空语义验收；贪吃蛇等网页游戏必须真实出现核心对象，空画布或只有界面框架不能再通过完成门禁。
- 用户指出“只有框架、缺少核心内容、未通过验收”时，决策内核会保留原任务目标并进入修复执行；网页产物修改后必须重新完成桌面与窄屏验收。
- 团队大厅恢复“新建团队”入口：标题区和空状态均可直接创建团队；Electron 优先打开独立工具窗口，不可用时回退到页内弹窗。
- 更新团队大厅与 Agent Kernel 回归脚本，覆盖创建入口可达、成员滚动、团队生命周期、任务插话接管与重复 Skill 调用收敛。

## v3.15.2 (2026-08-05)

- 将已审核通过的波普漫画 Demo 视觉差异收口到生产办公室、员工工牌、聊天、团队、任务观察、设置和弹层。
- 更新员工工牌的主题色循环、底栏状态、网点/网格背景和办公室汇总条，并补充视觉门禁检查。
- README 增加当前 UI 快照，GitHub 项目首页可直接查看本次验收画面。
- 发布本版本安装包和 `latest.yml`，让 3.15.1 客户端可以通过内置更新检查发现 3.15.2。

## v3.15.1 (2026-08-05)

- 将已审核通过的 `design-demos/pop-comic-ui` 作为生产界面的逐项视觉基准：波普主题采用全幅布局、62px 标题栏、4px 主分界、3px 次级控件、34px 正方形交互按钮、16px 面板圆角和 12px 控件圆角。
- 生产办公室、员工工牌、聊天、团队、任务观察、设置、诊断和 Ant Design 弹层统一使用 Demo 的网点背景、黑色描边、阴影、按钮层级和主题变量；Pop 主题不再保留旧版窗口内缩造成的 6px 空白。
- 标题栏按钮和聊天子窗口按钮统一为 Demo 的 34px 正方形线性图标控件；原版商务和酸性暗黑保持各自既有布局与主题规则，酸性暗黑继续隐藏员工牌顶部色块/挂带。
- 互动音效面板按 Demo 结构补齐 FC、Mac、街机、开关、音量和试听，默认开启、音量 80%；深色主题下记忆与诊断状态文字使用可读的前景色。
- 视觉门禁更新为 34px 控件和 Demo 根布局契约，覆盖 3 套风格、25 套配色及跨窗口视觉状态同步。

## v3.15.0 (2026-08-05)

- 修复助手、员工私聊和团队聊天窗口标题栏控件被拉成长方形的问题；锁定、最小化、最大化和关闭按钮现与主界面统一为 32 x 32px 正方形，并加入视觉契约回归检查。
- 新增长任务驻留检查点：跨客户端重启会核对目标、计划、完成步骤、验证证据、下一步骤、上下文摘要和 Worker 序号；一致时自动恢复，冲突时安全暂停并说明下一步。
- Turn Lifecycle 的上下文压缩、摘要、未决问题和用户插话纳入驻留一致性校验；用户主动继续可基于当前真实状态重建恢复基线，已完成步骤不重复执行。
- TaskService 新任务默认建立恢复上下文，Worker 领取旧任务时兼容补齐缺失上下文。
- `verify_web_artifact` 增加任务合同驱动的通用 `semanticChecks`，支持分组、顺序、相邻、网格坐标和关键交互；语义错误和布局错误一样阻断完成，不写科学计算器专用规则。
- 新增真实 Electron 语义回归和无界面长任务耐久回归，覆盖错误网格拒绝、修正后通过、两次上下文压缩、两次用户插话、跨会话恢复和连续五个检查点。
- 修复助手锁定层级：锁定时使用系统级置顶并在主窗口聚焦、窗口重建和显式打开后持续保持，解除锁定后恢复普通窗口层级；同样保护已锁定的员工/团队聊天窗口。
- 本版本完成 `v3.15` 路线目标；`v3.16` 继续事实版本、冲突证据、路线成功率和时间衰减。多窗口图形耐久测试在部分机器可能受 Electron 图形驱动环境影响，内核门禁与图形环境专项分开记录。

## v3.14.1 (2026-08-04)

- 将已定稿的风格化 Demo 迁移到生产客户端，提供原版商务、波普漫画和酸性暗黑三套风格、共 25 套配色。
- 主窗口、员工私聊、团队窗口、设置页、任务观察栏、员工工牌和 Ant Design 弹层共用视觉变量，跨窗口广播保持风格与配色一致。
- 新安装默认使用波普漫画；三种风格分别记住上次配色。原版商务保留 11 套原有主题和 1px 克制边框。
- 波普漫画统一使用小点状背景、4px 主边框和 3px 次级控件边框；酸性暗黑加入 4 套霓虹配色并隐藏工牌顶部挂带。
- 新增 FC、Mac 和街机三套互动音效，默认开启、音量 80%，支持关闭、调节和试听。
- 新增视觉目录单测、生产 CSS 契约门禁与办公室/团队真实快照；本版本只完成 UI 产品化迁移，不冒充 `v3.15` 长任务与恢复能力已完成。

## v3.14.0 (2026-08-04)

- 新增 `docs/TAIJI_V3_TO_V4_ROADMAP.md` 活路线表。后续每轮开发固定先回顾路线、代码证据与未完成项，再实施和交接。
- 新增统一 `autonomousExecutionGate`：真实工具动作必须同时绑定当前 `goalId`、计划版本、责任步骤、责任员工、工具名和已接受提案。
- 门禁会拒绝过期目标/计划、废弃或不可执行步骤、未完成依赖、错误责任员工、工具名不一致和绕过主持授权的直接调用。
- 助理、员工私聊、团队成员、团队交接文件读取与原生后台执行器使用同一协议；团队交接读取不再绕过自主决策权。
- 原生后台执行器在 `toolRuntime.execute`、动态子任务和 Git Worktree 工具真正执行前写入并消费同一份授权提案。
- 新增统一门禁、错误责任员工、聊天桥接和团队决策回归；Vitest `156/156`、原生执行专项、仓库卫生、模块/函数边界与生产构建通过。
- v4.0 尚未完成：唯一主持入口、正式 8 小时驻留、真实第三方账号矩阵、跨版本回滚和代码签名仍需后续验收。

## v3.13.0 (2026-08-04)

- 新增 `autonomousDecisionAuthority`，模型或运行时提出真实行动后，由确定性内核校验当前 `goalId`、计划版本、责任步骤、依赖、风险与授权；过期目标、过期计划和无效步骤会在工具执行前拒绝。
- 助理、员工私聊和团队员工的真实工具调用统一记录行动提案、公开理由、预期证据和责任步骤；没有新提案时不再生成虚假自主决策。
- TaskService 恢复命令、Agent Loop 最终收尾和团队自主决策分别拆入独立模块；`taskService.cjs` 降至约 477 行，`createTaskService` 降至约 180 行，Agent Loop 主函数约 730 行。
- 聊天任务完成链新增联动回归：只有责任步骤完成、`verify_completion` 决策通过且完成门禁通过后才关闭任务；证据不足保持等待，失败写回责任步骤。
- 分层记忆协议升级到 schema v2，正式增加情景记忆、语义记忆、程序记忆和用户偏好四类，并支持查询筛选、可读投影和记忆中心展示。
- 旧版用户事实与偏好兼容迁移；无法证明真实验收的旧任务经验只进入情景记忆，不冒充已验证程序路线。
- 任务复盘只有在任务状态完成且存在真实验证证据时才自动写入程序记忆；单纯模型声明、工具成功或空证据完成不会沉淀为可复用流程。
- 内置章北海人格升级到 v26，加入自主决策权、工具动作绑定、完成证据门禁与四类记忆协议。
- 新增/扩展自主决策、团队决策、TaskService 恢复、聊天桥接、四类记忆迁移和任务复盘专项测试；模块边界、函数边界、生产构建与 Lint 通过。
- 完整门禁通过：Vitest `152/152`、语义基准 `400/400`、阶段 A/B/3 与完整 `verify:v2-core-gate`。
- Windows 安装包 `taiji-office-setup-3.13.0.exe` 为 `195896339` 字节，SHA-256 `D7690E007BABE8D4A685880BCFCE4B380AEEA68865E8C1303B85DA13287AE515`；Blockmap SHA-256 `05662FE0E187322C529822C07197C3A3CFC7CBD97E27D6550B0375D584AC74BA`，`latest.yml` SHA-256 `3D1C081767DB288597C56814A499F2BA018B43D20F4159209A1AE209B2A0AEF5`。
- 已覆盖安装到 `%LOCALAPPDATA%\Programs\taiji-office`，产品版本 `3.13.0.0`、包内版本 `3.13.0`。覆盖前用户数据与备份均为 314 个文件、`222077920` 字节；实际安装模块将 184 条记忆迁移到 schema v2，客户端重启后 5 个进程全部响应。
...
- 成功写入大文件后只向模型历史保留路径和长度摘要，避免整份 HTML 在每轮重复占用上下文。
- 助理、员工和团队聊天任务统一保存工作区、工具证据、执行控制状态和收尾结果；前台回复不再被大账本落盘长时间阻塞。
- 大任务账本采用合并检查点和单任务增量投影，降低高频心跳造成的磁盘重写和界面卡顿。
- Web UI 验收新增桌面与不宽于 390px 的窄屏检查，以真实可用宽度而非 `innerWidth` 判断滚动条遮挡，并检查父容器裁切、边框、外阴影和 8px 安全区。
- 助理和员工私聊的“继续原任务”按会话、所有者和任务类型恢复原始目标与工作区，并在任务账本保存 `parentTaskId` 与本轮修订要求；失败续作不会再被当成新的孤立目标。
- 网页产出物修改后必须立即重新调用内置验收；单独检查桌面或窄屏不能通过完成门禁，必须在同一轮覆盖桌面与不宽于 480px 的窄屏。
...
- 新增 `GoalState`、`SituationModel`、`DecisionRecord` 与版本化 `AutonomousControlSnapshot`，建立 `observe -> interpret -> propose -> validate -> act -> verify -> reflect` 的统一控制协议。
- 新任务在创建时立即获得稳定 `goalId/projectId/conversationId`；旧任务通过追加式账本事件自动迁移，重启和恢复点还原后身份、证据、工作区、成员与步骤保持不变。
- `taskRuntimeStore` 成为助理、员工私聊、团队与后台任务共用的控制快照校准边界。旧渲染窗口缺少新字段时只能继承账本状态，不能擦除或反复重建自主记录。
- 用户纠正、补充约束与控制消息进入同一目标状态并去重；明确排队的独立新任务不会污染当前目标。已验证证据进入事实，未验证结果只能进入假设。
- 影子决策根据真实状态推荐规划、开始、继续、验证、等待、反思或换路线；同一路线连续失败两次会标记重复并建议实质性换路。
- 团队任务和项目详情新增“自主判断”公开摘要，显示下一步、理由、阻塞、事实、尝试路线和预期证据。该摘要可审计，但不包含隐藏思维链。
- 本版不替换现有执行器，不宣称已完成全自主执行。后续真实任务将对比旧执行路线与影子建议，再逐步进入动态计划和自主恢复阶段。
- 全量单元测试 `96/96`、自主控制专项、任务账本重启/旧版迁移、TypeScript、Lint、生产构建、模块边界与完整 `v2` 核心门禁通过。
- Windows 安装包 `taiji-office-setup-3.6.0.exe` 为 `195873370` 字节，SHA-256 为 `3EDA068868A5F8FE67B1CAAEE327D9F99F8E3CF8F838B704FE8B93F2AF2DFB53`；包内版本、自主控制模块、公开判断界面和六款字体均通过校验。
- 已备份本机用户数据并覆盖安装，安装目录 `app.asar` 的真实版本读取结果为 `3.6.0`；覆盖前备份位于 `L:\AI办公室\taiji-backups\preinstall-3.6.0-20260803-2316`。

## v3.5.8 (2026-08-02)

### Windows 黑屏恢复与渲染诊断

- Windows 客户端默认使用稳定的软件渲染，避开已确认会让 Electron 页面纯黑的本机 GPU 合成链；可用 `TAIJI_FORCE_HARDWARE_ACCELERATION=1` 临时恢复硬件加速做诊断。
- 主窗口、助理、员工私聊、团队聊天、设置和独立工具窗口统一改为页面加载后再显示，避免先出现空白或黑色原生窗口。
- 所有窗口统一记录主页面加载失败、渲染进程退出、窗口无响应和严重控制台错误；加载事件异常时由十秒启动兜底显示窗口，不再留下没有界面也没有日志的后台进程。
- 新增渲染策略与窗口显示生命周期回归测试。软件渲染下已通过 Chrome DevTools Protocol 读取真实安装版页面，确认 React、员工数据和助理界面完整渲染。
- 真实新建“手机生图 APP”项目时发现迁移后的宽泛能力标签会让项目经理或工程师冒充 UI 岗位。现在协调、架构、UI、前端、后端和 QA 在办公室有对应专家时必须由专业身份覆盖，宽泛标签只能作为辅助能力。
- Coding DAG 与组队阶段现在复用同一套专业岗位所有权规则，不会在团队名单正确后再次按宽泛标签重选负责人。产品简报和最终交付归项目管理人员，架构、UI、前后端和审查分别归对应专业员工；新增真实六人污染标签名单的八步负责人回归测试。
- 固定 Coding DAG 不再向模型开放会复制既有职责的动态委派工具；正式审查收到结构化 `PASS/REJECT` 后立即进入下一状态，不再提交结论后继续空转模型轮次或把八步计划扩成十余步。
- 工具注册进一步按步骤职责收紧：`submit_review` 只对正式审查步骤可见，产品、设计、实现和交付步骤在满足自身答案/文件/运行证据后直接总结完成，不能再用审查工具替代本步骤收尾。
- 新建聊天只显示当前 `conversationId` 的待批准项目卡；旧会话审批卡、驳回草案和运行项目不再进入新会话的显示与调度解析，未带会话字段的历史项目只留在旧版兼容会话中。
- 文件交付步骤产生真实落盘文件并保留成功运行验证后，会依据证据直接完成；即使刚好到达 24 次工具预算，也只停止重复路线并按已有证据收口，不再把已完成实现误判为失败。
- 长文件生成允许在已经出现真实写入或运行进展时进入受限收尾轮次；模型请求、超时、重试和控制中断统一移入 `nativeModelGateway.cjs`，Adapter 本身继续满足模块边界门禁。
- 首次任务账本同步完成前禁止自动恢复，避免主进程用尚未同步的旧投影启动项目。动态生成的复审步骤固定为 `decision`，所有正式 `review` 步骤提交结论后立即结束。
- 使用真实六人团队完成一次手机生图 APP 项目运行：产品、架构、UX/UI、前端、后端、验证、首次审查和第一次修订均已执行，工作区真实产生 18 项文件与浏览器证据；系统正确退回缺少图生图、设置持久化和完整浏览器证据的交付，没有虚构验收通过。最终复审因模型服务连续返回 HTTP 502 暂停，现场可恢复，Delivery 未执行。

## v3.5.7 (2026-08-02)

### 工作区能力事实校正

- 章北海回复前会读取当前项目、父子任务和真实工作区状态；子任务未重复保存工作区字段时，沿项目父链继承，不再误判为没有写入入口。
- 当工作区已经建立且任务仍有未完成步骤时，模型若仍声称“没有工作区写入或运行验证入口”，内核会在消息落地前改为真实完成数、当前负责人、步骤状态和实际阻塞原因。
- 原生 Worker 与备用团队执行链同步获得工作区能力事实：尚未产生文件只表示当前步骤尚未真实写入或验证，必须调用工具并报告真实结果，不能解释成没有工具入口。
- 新增父子项目工作区继承与错误回复校正回归测试。

## v3.5.6 (2026-08-02)

### 软件项目恢复与真实交付

- “继续”“恢复任务”“立即进入原型实现阶段”等确认语句，在当前会话确有暂停、失败或等待确认的未完成项目时，会恢复同一根项目及其工作区，不再进入没有文件工具的普通聊天通道；否定表达不会误触发恢复。
- 修复父任务恢复子任务时的入队竞态：先将持久化状态切回可执行，再加入内存队列，避免出现“任务状态 awaiting_user 不能领取执行租约”。
- 软件开发项目的根验收合同改为真实交付：必须产出经过磁盘回读的源文件、保留成功运行或测试证据，并在最终交付中列出文件路径、验证结果和未决限制；规划文字不能代替代码成品。
- 团队主持上下文加入真实工作区，并禁止在已有工作区和未完成实现阶段时误报“没有可写入或运行入口”。
- 回归测试覆盖自然语言继续、否定继续、暂停子任务恢复时序和软件项目真实交付合同。

## v3.3.0 (2026-08-01)

### 团队主持与阶段交接

- 章北海成为团队群聊的常驻主持人：未明确点名员工时优先结合当前项目、活动阶段、负责人、等待条件和已完成证据回应；项目澄清阶段可以继续沟通，但不会提前启动员工工具。
- 用户插话继续使用模型语义分类为询问、纠正、补充约束、暂停、继续或新目标；同一项目的后续调整继承项目根、工作区、附件和完成证据，不创建平行项目。
- 原生执行器和渲染备用执行器统一生成阶段交接卡，固定展示“解决什么、为什么这样做、已经做到、还没有做、下一步由谁执行”；底层工具过程默认折叠。
- 审查退回会明确责任员工、责任步骤和重新验收动作，不再只输出笼统拒绝结果。

### 授权、导出与模块边界

- 授权卡展示申请人、目的、动作、读写范围、风险及批准/拒绝效果；批准后刷新原卡状态并继续，拒绝后持久禁止原样再次申请同一动作。
- 团队聊天导出使用“章北海助理 / 常驻主助理”身份，并结构化保存阶段交接、授权决定、附件引用和折叠执行过程。
- 新增 `nativeCollaborationProtocol.cjs`、`teamSupervisor.ts`、`teamStageHandoff.ts` 与 `teamControl.ts`，将主持、授权和阶段交接从超大执行器与 Store 中拆出。
- 修复总设置入口仍使用人格 v21 迁移附录的问题，所有助理人格入口统一使用 v22；会话隔离门禁同步验证新拆分的原生协作协议。
- 内置章北海人格升级到 v22。标准测试 `68/68`、400 条语义基准 `400/400`、完整 v2 核心门禁、团队成员连续性、原生执行器、模块边界、TypeScript、Lint 与生产构建通过。
- Windows 安装包 `taiji-office-setup-3.3.0.exe` 为 `195851435` 字节，SHA-256 `8C963B1051A5151A433DE81BEF41973E89A51B9E8D62CBF82EFB83037A7CB601`；Blockmap 为 `207381` 字节，SHA-256 `B5C0D5A38E1821D05812E81E4A2841B8A6919E2D185E58BD84BA9C6EB0A1FCD5`；`latest.yml` 为 `353` 字节，SHA-256 `A7C0873ED006F6F1BBA8ACD7535733362853742052B1BD59C918873A9DDBCA30`。

## v3.2.1 (2026-08-01)

### 上下文连续性修复

- 收紧前置资源指代解析：普通的“这个配置”“这个项目”“按第二次提议”等语义不再被误判成文件或 Skill 歧义，只有真实读取、安装和链接请求才进入资源绑定。
- 助理和员工私聊的有效历史窗口扩大；任务决策层保留更多最近对话并提高单条上下文上限，减少忘记上一轮回复和把纠正句当成新任务的问题。
- 团队调度会恢复原始项目目标和助理最近的专业人员方案；安卓、移动开发、AI 模型接入、后端、审查与连接能力进入统一能力图。
- 被驳回的团队草案可以按用户明确引用的修订方案原子恢复并批准，不再误报“没有可批准草案”。

### 聊天附件导出

- 员工私聊用户消息现在保存真实附件对象；助理、员工私聊和团队聊天导出统一包含附件名称、类型、大小、保存状态和工作区路径。
- 图片附件使用落盘路径生成 Markdown 引用，不把大体积 Base64 写入导出文件；文本附件附带有限预览和截断说明。
- 标准测试 `65/65`、400 条任务语义基准 `400/400`、V2 核心门禁、生产构建和 Lint 均通过。
- Windows 安装包 `taiji-office-setup-3.2.1.exe` 为 `195847308` 字节，SHA-256 `2C871138D5D92017D5193683029035890C7A63D40D0A71CE4DEC2C0C6E4CE507`；包内版本和 `latest.yml` 均为 `3.2.1`。

## v3.2.0 (2026-08-01)

### 长任务与项目看板

- 项目看板新增负责人、阶段/步骤耗时、证据完整度、等待条件、下一步和责任返工范围；子任务、重试和恢复继续归入同一根项目。
- 将本地状态持久化、员工运行状态投影和原生任务公开投影从超大核心模块抽离，并收紧模块行数门禁。

### 性能与多窗口

- 渲染层按专家目录、React 和 Ant Design 分包，主 JS 从约 5.14 MB 降至约 0.81 MB，下降 84.1%；清除 5 条无效动态导入警告。
- 性能门禁覆盖 320 员工、12000 事件、40 项目/3200 步骤和 12 窗口事件广播。真实桌面软件渲染短驻留打开 12 窗口，堆增长 0.15 MB。
- 修复 E2E 在 preload 尚未就绪时立即调用窗口 API 的竞态；GPU 硬件与软件降级路径分开记录。

### 人格与验证

- 内置章北海人格升级到 v21，增加长任务、状态投影、阶段看板和长期测试证据边界。修复助手聊天入口只迁移基础附录、可能漏掉阶段协议的问题。
- 标准测试 `58/58`，项目看板、模块边界、规模性能、人格、无警告构建和 12 窗口短驻留通过。
- Windows 安装包与包内验收通过：`taiji-office-setup-3.2.0.exe` 为 `195846389` 字节，SHA-256 `32266E279020CB74F6284D7A8F37853C56D3B3CFA95EDB88A06E17F0BE0A4BE0`；Blockmap 为 `206117` 字节，SHA-256 `8129C43B6631521D913917BDDEE2A9A7946048F1E2216A4E363E94D03DEE5E5B`；`latest.yml` 为 `353` 字节，SHA-256 `EF155F850513B6259E10AC6BF0DD482AE8FB1EDB43822E7BC1C83B0ED368F9CF`。
- 未声称完成：正式 8 小时驻留、真实第三方账号全矩阵、代码签名、真实跨版本回滚和 GitHub 远端发布。

## v3.1.0 (2026-08-01)

### 外部能力矩阵

- 新增九类外部能力统一清单：聊天模型、图片生成、指定网页、SkillHub、知识库、邮件、GitHub、HTTP 和 MCP。没有配置的类别也进入矩阵并显示缺配置，避免诊断漏报。
- 新增八态真实验证合同：缺配置、未测试、真实可用、鉴权失败、限流、协议错误、内容无效和不可用。恢复次数只在真实失败后再次真实调用成功时增加。
- 模型/图片兼容性测试、连接器测试、网页读取、SkillHub 与连接器工具调用统一回写脱敏证据。配置保存、工具发现和 Skill 安装不能把状态改成可用。
- 诊断中心增加响应式矩阵视图；窄窗口下模型选择、诊断按钮和能力条目自动分行，不再被右侧裁切。

### Skill 与人格

- Skill 证据协议升级到 v3：发现、读取规则、真实调用、产生结果和最终验收五段分开记录；助手、员工私聊和团队任务统一接入。
- 修复安装成功被误记为已经调用的问题。聊天消息现在区分已安装、已产出、已验收和未通过验收。
- 内置章北海人格升级到 v20，加入外部能力真实验证协议；旧自定义人格按缺失章节增量迁移。

### 验证与边界

- 标准测试 `57/57`，400 条语义轨迹 `400/400`；`verify:phase-a`、`verify:phase-b`、`verify:v2-core-gate`、`verify:phase3` 与生产构建通过。
- 阶段 A 的 `v3.0.0` Windows 安装包已生成并通过包内验收。`v3.1.0` 当前为源码阶段，尚未打包、安装或发布 GitHub Release。
- 未声称完成：使用真实用户账号验证九类能力、邮件/GitHub 写操作副作用测试、正式 8 小时驻留、代码签名和真实跨版本回滚。

## v3.0.0 (2026-08-01)

### 语义决策

- 新增四层任务决策审计：候选理解、上下文约束、风险/证据校验和可执行计划分别保存有限输入、接受结果和拒绝原因；审计随团队任务合同持久化，不记录隐藏思维过程。
- 语义基准扩展到 400 条并将门槛提高到 98%。修复“停止执行”词序未被识别，以及明确团队任务被通用命令路由提前截获的问题；当前基准为 `400/400`。

### 模型韧性与观测

- 新增模型运行期健康登记：连续瞬时故障触发熔断，冷却后只允许半开探测，成功后关闭保护；503、429、超时和网络中断分别统计。
- 新增确定性指数退避、备用聊天模型建议和受控降级。系统不静默切换模型；保护窗口保留当前任务现场并阻止重复请求。
- 模型指标记录请求数、成功率、总耗时、首 token、HTTP 状态、失败分布和恢复结果；诊断中心展示运行期摘要，模型身份和错误记录不包含 API Key。
- 故障注入回归验证 503 连续失败后第 4 次不再请求，并分别验证 429、超时和网络中断分类。

### 人格与门禁

- 内置章北海人格升级到 v19，加入四层语义判断、当前消息优先、模型保护窗口、禁止静默切换和指标边界。
- 新增 `verify:task-semantics-benchmark`、`verify:task-decision-pipeline`、`verify:model-reliability` 和统一 `verify:phase-a`。
- 将聊天上下文整理与模型请求韧性从 `hermesClient.ts` 拆到独立模块；标准测试 `49/49`、v2 核心门禁、阶段三工程门禁、Lint 和生产构建通过。
- 未声称完成：真实供应商长时间故障恢复、8 小时 Electron 驻留、第三方账号矩阵、真实跨版本回滚和远端发布资产核验。
...
- Stage E：升级失败注入与回滚演练。
- 凭据增加过期、轮换、权限范围和最小权限审计。
- 增加发布一致性门禁、SBOM 与 provenance 版本校验。
# v3.5.0

- Stage F：项目级协作交付模型，支持成员职责、阶段依赖、审查退回、风险登记、变更记录和验收包导出。
# v3.5.1

- 修复后台助手任务无可见提示、跨新聊天误接旧请求和 Markdown 消息未渲染。
# v3.5.2

- 项目面板增加一键导出当前团队会话全部任务回放 Markdown。
# v3.5.3

- 团队实时过程面板支持折叠，折叠后保留当前任务状态且不影响后台执行。
# v3.5.4

- 助理的任务确认信息高亮并绑定当前任务；点击整段即批准继续，插话进入同任务重新评估链。
# v3.5.5

- 折叠控件移到实时过程面板左侧，避免与暂停、停止按钮重叠。
