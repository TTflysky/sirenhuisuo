...
> 最后整理：2026-08-08
> 当前源码版本：`v5.10.4`；任务账本支持超大文件流式恢复，暂停和事实账本状态可持续恢复
> 主分支：`main`
> 仓库：[TTflysky/sirenhuisuo](https://github.com/TTflysky/sirenhuisuo)

## v5.10.4 当前交接（2026-08-08）

- 超过 64MB 的任务账本按快照水位流式重放并归档，不再使用 `readFile` 把整个账本载入内存；暂停、停止、关闭在账本不可读时仍可生效。
- 事实账本使用证据水位避免重启重复生成冲突；明确交付物文件名和测试报告保持在项目契约中。
- 回归验证：59 个测试文件、197 项测试通过，`verify:v510` 通过。

## v5.10.3 历史交接（2026-08-08）

- 累计 Token 达到任务预算时进入 `checkpointed`，已有真实证据仍可完成最终验收；没有足够证据时暂停并保留恢复上下文。
- 恢复时重置当前预算周期并保留跨周期 `lifetimeUsage`；团队运行观察把容量检查点显示为暂停，而不是成员失败。
- 回归验证：58 个测试文件、195 项测试通过；V5 自评见 `docs/TAIJI_V5_SELF_ASSESSMENT.md`。

## v5.10.2 历史交接（2026-08-08）

- 运行观察团队聊天不再强制使用固定浅色画布，而是继承当前主题的语义背景、表面、边框和前景色；深色、波普漫画与酸性暗黑主题不再出现浅底浅字。
- 视觉主题回归验证禁止运行观察壳重引入固定浅色令牌，并确认运行观察私有输入区样式不会污染普通聊天。

## v5.10.0 历史交接（2026-08-08）

- 新项目优先从模型任务决策中的交付物、验收、依赖和能力要求生成动态 DAG；无依赖交付物有限并行，整合节点等待全部责任交付物证据。
- 成员步骤携带结构化任务合同，并由 TaskService、自适应计划图和责任子任务统一持久化；不建立第二套团队或产物事实源。
- 项目草案与团队聊天均按全部必需交付物的能力并集选择成员；能力缺口会显式记录，不以通用成员掩盖。
- `verify:v510-deliverable-kernel`、自适应计划、TaskService、完成门、Lint、生产构建和完整 190 项单元回归均已通过。

## v5.9.1 历史交接（2026-08-08）
